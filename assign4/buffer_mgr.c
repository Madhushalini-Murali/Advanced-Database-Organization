...
{
	if (bm->numPages >= bufferManager->calculate || bm->numPages != bufferManager->calculate )
	{
...
{
	if (bm->numPages < mgmt->calculate || bm->numPages != mgmt->calculate)
	{
		CheckIfBufferPoolIsEmpty(pageNumber, bm);
