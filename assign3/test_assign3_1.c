...
static void testSchemaTooBig(void);
static void testRecordTooBig(void);
...
Schema *testLargeSchema (void);
Record *fromTestRecord (Schema *schema, TestRecord in);
Schema *testLargeRecordSchema(void);
...
void
testSchemaTooBig(void)
{
  RM_TableData *table = (RM_TableData *) malloc(sizeof(RM_TableData));
  Schema *schema;
  testName = "test creating a new table with schema larger than pageSize minus 16 bytes";
  schema = testLargeSchema();
...
void
testRecordTooBig(void)
{
  RM_TableData *table = (RM_TableData *) malloc(sizeof(RM_TableData));
  Schema *schema;
  testName = "test creating a new table and inserting a record larger than DataPage->data size";
  schema = testLargeRecordSchema();
...
Schema *
testLargeRecordSchema (void)
{
  Schema *result;
  // (page size) - (prev pointer) - (next pointers) - int (for spanned records)
  // - (1 byte for tombstone) + (1 byte to go over)
  int overMax = PAGE_SIZE - 3 * 4 - 1 + 1;   
  char *names[] = { "a" };
  DataType dt[] = {  DT_STRING };
  int sizes[] = { overMax };
  int keys[] = {0};
  int i;
  char **cpNames = (char **) malloc(sizeof(char*) );
  DataType *cpDt = (DataType *) malloc(sizeof(DataType) );
  int *cpSizes = (int *) malloc(sizeof(int) );
  int *cpKeys = (int *) malloc(sizeof(int));
  for(i = 0; i < 1; i++)
    {
      cpNames[i] = (char *) malloc(2);
  ...
  memcpy(cpDt, dt, sizeof(DataType));
  memcpy(cpSizes, sizes, sizeof(int));
  memcpy(cpKeys, keys, sizeof(int));

  result = createSchema(1, cpNames, cpDt, cpSizes, 1, cpKeys);

  return result;
}

Schema *
testLargeSchema (void)
{
  ...
  char **cpNames = (char **) malloc(sizeof(char*) * overMax);
  DataType *cpDt = (DataType *) malloc(sizeof(DataType) * overMax);
  int *cpSizes = (int *) malloc(sizeof(int) * overMax);
  int *cpKeys = (int *) malloc(sizeof(int));
  ...
  memcpy(cpDt, dt, sizeof(DataType) * overMax);
  memcpy(cpSizes, sizes, sizeof(int) * overMax);
  memcpy(cpKeys, keys, sizeof(int));

  result = createSchema(overMax, cpNames, cpDt, cpSizes, 1, cpKeys);
  return result;
}
